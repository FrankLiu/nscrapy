...
Date.prototype.toMoment = function (format) {
	if (!this.__moment) {
		if (_.isEmpty(format)) this.__moment = moment(this);else this.__moment = moment(this, format);
	}
...
Function.logFn = function () {
	console.log(arguments);
};

//====================================================underscore函数扩展
_.mixin({
	isNotEmpty: function isNotEmpty(o) {
		return !_.isEmpty(o);
	}
});
...
function colorable(type, msg) {
	...
	return '\x1B[' + (ANSI_CODES[type] || ANSI_CODES['INFO']) + 'm' + msg + '\x1B[0m';
};
...
var RuntimeError = function (_Error) {
	...
	function RuntimeError(code, msg) {
		_classCallCheck(this, RuntimeError);

		var _this = _possibleConstructorReturn(this, (RuntimeError.__proto__ || Object.getPrototypeOf(RuntimeError)).call(this, msg));
...
scrapy = _.extend({}, {
	...
	//生成全局唯一id
	genUuid: function genUuid(timeBased) {
		if (timeBased) return uuid.v1();else return uuid.v4();
	},

	//抛出异常
	raiseError: function raiseError(code, msg) {
		return new RuntimeError(code, msg);
	},

	//安全执行
	tryCatch: function tryCatch(lambda, callback) {
		var result = {};

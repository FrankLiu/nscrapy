...
HttpClient.prototype = {
    ...
    setReferer: function setReferer(referer) {
        this.reqOptions.headers['Referer'] = referer;
    },

    //设置headers['User-Agent']属性
    setUserAgent: function setUserAgent(ua) {
    ...
    //发送请求并分析结果
    _call: function _call(method, url, options, callback) {
        ...
        if (options.cookies && _.isArray(options.cookies)) {
            ...
            try {
                for (var _iterator2 = this.cookies[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    var c = _step2.value;
    ...
    //处理返回结果
    _onResponse: function _onResponse(resp, options, url) {
        var self = this;
        //header handler
    ...
    //自动检测encoding，内部使用
    _detectEncoding: function _detectEncoding(contentType) {
        var encoding;
        if (contentType) {
    ...
    //解析网页内容，内部使用
    _parseContent: function _parseContent(content, contentType) {
    ...
    //post请求
    post: function post(url, options, asyncCallback) {
        this._call('POST', url, options, asyncCallback);
        return this;
    },

    //put请求
    put: function put(url, options, asyncCallback) {
        this._call('PUT', url, options, asyncCallback);
        return this;
    },

    head: function head(url, options, asyncCallback) {
        this._call('HEAD', url, options, asyncCallback);
        return this;
    },

    //opts property: autoDecode
    enableAutoDecode: function enableAutoDecode() {
        this.opts.autoDecode = true;
    },
    disableAutoDecode: function disableAutoDecode() {
        this.opts.autoDecode = false;
    },
    //opts property: keepCookies
    enableKeepCookies: function enableKeepCookies() {
        this.opts.keepCookies = true;
    },
    disableKeepCookies: function disableKeepCookies() {
        this.opts.keepCookies = false;
    },

    //事件链，模拟多个url采集参数传递
    pipe: function pipe(dst) {
        var self = this;
    ...
    //回调链，可用于多个请求之间的结果传递
    waterfall: function waterfall(chains, callback /*所有函数完成后返回*/) {
        var self = this;
        var fns = [],
            chain;
